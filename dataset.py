...
def load_planetoid_dataset(data_dir, name):
    ...
    dataset.train_idx=idx[:train_num]
    dataset.valid_idx=idx[train_num:val_num]
    dataset.test_idx=idx[val_num:]
...
def get_dataset(path, name):
    assert name in ['Cora', 'CiteSeer', 'PubMed', 'DBLP', 'Karate', 'WikiCS', 'Coauthor-CS', 'Coauthor-Phy',
                    'Amazon-Computers', 'Amazon-Photo', 'ogbn-arxiv', 'ogbg-code','ogbn-proteins','Squirrel','Chameleon']
    ...
    if name == 'Squirrel':
        return WikipediaNetwork(root=path,name=name,geom_gcn_preprocess=True,transform=T.NormalizeFeatures())
    if name == 'Chameleon':
        return WikipediaNetwork(root=path,name=name,geom_gcn_preprocess=True,transform=T.NormalizeFeatures())
...
def load_dataset_all(data_dir, name,sub_dataname=''):
    transform = T.NormalizeFeatures()
    torch_dataset = get_dataset(path=data_dir,name=name)
    # torch_dataset = Planetoid(root=f'{DATAPATH}Planetoid', name=name)
    data = torch_dataset[0]
    ...
    if name in ['Cora','CiteSeer','PubMed']:
        ...
        dataset.valid_idx=idx[train_num:val_num]
        dataset.test_idx=idx[val_num:]
    print("shape==",dataset.train_idx.shape,dataset.valid_idx.shape,dataset.test_idx.shape)
    dataset.graph = {'edge_index': edge_index,
                     'node_feat': node_feat,
                     'edge_feat': None,
                     'num_nodes': num_nodes}
...
def load_graph_classification_dataset(dataset_name, deg4feat=False):
    ...
    path=os.path.join("embed",dataset_name)
    embed=torch.load(path+".pt")
    for i, g in enumerate(dataset):
        dataset[i].edge_index = remove_self_loops(dataset[i].edge_index)[0]
        dataset[i].edge_index = add_self_loops(dataset[i].edge_index)[0]
        dataset[i].x=embed[i].cpu()
    #graph.num_features=(int)(embed[0].shape[1])
    #dataset = [(g, g.y) for g in dataset]
    feature_dim = (int)(embed[0].shape[1])
