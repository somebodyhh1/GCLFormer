...
def parse_method(args, dataset, c, d, device):

    if args.method=='mygcn':

        model=Model(d,args.hidden_channels,args.hidden_channels).to(device)
        ...
        model=NodeFormer(d, args.hidden_channels, c, num_layers=args.num_layers, dropout=args.dropout,
                    ...
                    nb_gumbel_sample=args.K, rb_order=args.rb_order, rb_trans=args.rb_trans,sigma=args.sigma,clustering=clustering).to(device)
    else:
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--task',default='node_classification',
                        choices=['node_classification','link_prediction','clustering'])
    ...
    parser.add_argument('--rand_split',default=True, action='store_true', help='use random splits')
    ...
    parser.add_argument('--use_gumbel',default=False, action='store_true', help='use gumbel softmax for message passing')
    parser.add_argument('--use_residual',default=True, action='store_true', help='use residual link for each GNN layer')
    parser.add_argument('--use_bn',default=True, action='store_true', help='use layernorm')
    parser.add_argument('--use_act',default=False, action='store_true', help='use non-linearity for each layer')
    ...
    parser.add_argument('--tau1', type=float, default=0.25, help='temperature for gumbel softmax')
    parser.add_argument('--lamda', type=float, default=1, help='weight for edge reg loss')
    parser.add_argument('--rb_order', type=int, default=3, help='order for relational bias, 0 for not use')
