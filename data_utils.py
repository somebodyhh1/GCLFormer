...
def get_direct_neighbors(edge_index,num_nodes):
    direct_neighbor=[[]]*num_nodes
    lengths=[0]*num_nodes
    _,e=edge_index.shape
    for i in range(e):
        direct_neighbor[edge_index[0,i]].append(edge_index[1,i])
        lengths[edge_index[0,i]]+=1
    min_length=np.min(lengths)
    max_length=np.max(lengths)
    copy_length=deepcopy(lengths)
    num_loops=(int)(np.ceil(1.0*max_length/min_length))
    print("num_loops==",num_loops)
    temp=deepcopy(direct_neighbor)
    for loop in range(num_loops):
        print("loop==",loop,len(direct_neighbor))
        for i in range(num_nodes):
            if lengths[i]>=max_length/100:
                continue
            direct_neighbor[i].extend(temp[i])
            lengths[i]+=copy_length[i]
    print("tmp1")
    direct_neighbor=[i[0:max_length] for i in direct_neighbor]
    print("max_length==",max_length,len(direct_neighbor))
    print("tmp2")
    direct_neighbor=torch.tensor(direct_neighbor)
