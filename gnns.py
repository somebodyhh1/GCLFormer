...
from myGCN import Encoder    
class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
                 dropout=0.5, save_mem=True, use_bn=None):
        super(GCN, self).__init__()
        out_channels=hidden_channels
        self.convs = nn.ModuleList()
        self.convs.append(GCNConv(in_channels, 2*hidden_channels))
        for _ in range(num_layers-1):
            self.convs.append(GCNConv(2*hidden_channels, 2*hidden_channels))
        self.convs.append(GCNConv(2*hidden_channels, out_channels))
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor,tau=0.5):
